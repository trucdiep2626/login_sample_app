import 'package:flutter/material.dart';

class AppColors {
  static List<Color> orangeGradient = [
    const Color(0xFFFF9C01),
    const Color(0xFFFF9B00),
    const Color(0xFFFFD33B),
  ];

  static List<Color> yellowGradient = [
    const Color(0xFFFF9C01),
    const Color(0xFFFF9C00),
    const Color(0xFFFFD33C),
  ];

  static List<Color> greenGradient = [
    const Color(0xFF4AC400),
    const Color(0xFF97CD00),
  ];

  static List<Color> redGradient = [
    const Color(0xFFFA0C00),
    const Color(0xFFFE6A00),
  ];
  static const Color primary = Color(0xFF1332B5);
  static const Color text = black;
  static Color background = const Color(0xffffffff);
  static Color greyBackground2 = const Color(0xffefefef);

  static const Color icon = Color(0xFF98A3B3);

  static const Color white = Colors.white;

  static const Color title = Color(0xFF222222);
  static const Color blue = Color(0xff339beb);
  static const Color yellow = Color(0xFFFFD231);
  static const Color purple = Color(0xffa133ed);

  //green
  static const Color green = Color(0xff00BB86);
  static const Color green2 = Color(0xff1b5d1f);

  //green
  static Color green50 = Colors.green.shade50;
  static const Color red50 = Color(0xffFFEBEE);
  //orange
  static Color orange50 = Colors.orange.shade50;

  //orange

  // static const Color orange2 =  Color(0xffFE9900);
  // static const Color orange = Color(0xffF99D12);

  //red
  static const Color red = Color(0xffFF3B31);

  static const Color danger = Color(0xffFD5F62);

  static Color transparent = Colors.transparent;

  static const Color backgroundItem = Color(0xffFFF5E5);

  static Color? grey200 = Colors.grey[200];
  static Color? grey300 = Colors.grey[300];

  //black
  static const Color black = Colors.black;
  static const Color black45 = Colors.black45;
  static Color black54 = Colors.black54;
  static Color stroke = const Color(0xffDDDDDB);
  static Color greyBackground = const Color(0xfff8f8f8);
  static Color greyBorder = const Color(0xff808080);

  //grey
  static const Color grey0 = Color(0xffB0B0B0);
  static const Color grey0_5 = Color(0xffa1a1a1);
  static const Color grey1 = Color(0xff929292);
  static const Color grey1_5 = Color(0xff848484);
  static const Color grey2 = Color(0xff757575);
  static const Color grey3 = Color(0xff575757);
  static const Color grey4 = Color(0xff3a3a3a);
  static const Color grey5 = Color(0xff1c1c1c);
  static const Color grey6 = Color(0xff161616);
  static const Color grey7 = Color(0xff111111);
  static const Color grey8 = Color(0xff0b0b0b);
  static const Color grey8_5 = Color(0xff080808);
  static const Color grey9 = Color(0xff060606);
  static const Color grey9_5 = Color(0xff030303);
  static const Color grey10 = Color(0xff000000);

  //orange
  static const Color orange = Color(0xffF99D12);
  static const Color orange0_5 = Color(0xffFFF5E6);
  static const Color orange1 = Color(0xffFFEBCC);
  static const Color orange1_5 = Color(0xffFFE0B3);
  static const Color orange2 = Color(0xffFFD699);
  static const Color orange3 = Color(0xffFEC266);
  static const Color orange4 = Color(0xffFEAD33);
  static const Color orange5 = Color(0xffFE9900);
  static const Color orange6 = Color(0xffCB7A00);
  static const Color orange7 = Color(0xff985C00);
  static const Color orange8 = Color(0xff663D00);
  static const Color orange8_5 = Color(0xff4C2E00);
  static const Color orange9 = Color(0xff331F00);
  static const Color orange9_5 = Color(0xff190F00);
  static const Color orange10 = Color(0xff000000);

  //blue
  static const Color blue0 = Color(0xffB0B0B0);
  static const Color blue0_5 = Color(0xffa1a1a1);
  static const Color blue1 = Color(0xff929292);
  static const Color blue1_5 = Color(0xff848484);
  static const Color blue2 = Color(0xff757575);
  static const Color blue3 = Color(0xff575757);
  static const Color blue4 = Color(0xff3a3a3a);
  static const Color blue5 = Color(0xff1c1c1c);
  static const Color blue6 = Color(0xff161616);
  static const Color blue7 = Color(0xff111111);
  static const Color blue8 = Color(0xff0b0b0b);
  static const Color blue8_5 = Color(0xff080808);
  static const Color blue9 = Color(0xff060606);
  static const Color blue9_5 = Color(0xff030303);
  static const Color blue10 = Color(0xff000000);

  //purple
  static const Color purple0 = Color(0xffB0B0B0);
  static const Color purple0_5 = Color(0xffa1a1a1);
  static const Color purple1 = Color(0xff929292);
  static const Color purple1_5 = Color(0xff848484);
  static const Color purple2 = Color(0xff757575);
  static const Color purple3 = Color(0xff575757);
  static const Color purple4 = Color(0xff3a3a3a);
  static const Color purple5 = Color(0xff1c1c1c);
  static const Color purple6 = Color(0xff161616);
  static const Color purple7 = Color(0xff111111);
  static const Color purple8 = Color(0xff0b0b0b);
  static const Color purple8_5 = Color(0xff080808);
  static const Color purple9 = Color(0xff060606);
  static const Color purple9_5 = Color(0xff030303);
  static const Color purple10 = Color(0xff000000);

  //neutral
  static const Color neutral300 = Color(0xffc0c0cf);
}
